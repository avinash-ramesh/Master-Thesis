...
function [particles] = predict(particles, T, velocity, heading)
    std_velocity = 1; %KF both scenes - 1; EKF
    std_heading = 0.1; %KF both scenes - 0.08; EKF
    for p = 1:size(particles,2) % Use Parallel toolbox
%         particles(p).X(1) = particles(p).X(1) + (T*(velocity + (randn(1)*std_velocity))*cos(heading + (randn(1)*std_heading)));
%         particles(p).X(2) = (velocity + (randn(1)*std_velocity))*cos(heading + (randn(1)*std_heading));
%         particles(p).X(3) = particles(p).X(3) + (T*(velocity + (randn(1)*std_velocity))*sin(heading + (randn(1)*std_heading)));
%         particles(p).X(4) = (velocity + (randn(1)*std_velocity))*sin(heading + (randn(1)*std_heading));
    ...
    for m = 1:size(Z.id,2)
        if global_point_landmark_map.isKey(string(Z.id(m))) 
            for p = 1:size(particles,2)
                ...
                R = blkdiag(5,0.01); %EKF scene0655 :0.6,0.01 or 0.5,0.01 ; EKF scene0916: 0.8,0.07 or 0.8,0.05  multi_0916: 0.1, 0.005; one_0655:5,0.001 ; multi_0655: 0.1, 0.005
