...
%     std_heading = 0.01; 
    ...
    for p = 1:size(particles,2) % Use Parallel toolbox
        trans = randn(1)*std_velocity;
        rot = randn(1)*std_heading;
        particles(p).X(1) = particles(p).X(1) + (T*(velocity + trans)*cos(heading + rot));
        particles(p).X(2) = (velocity + trans)*cos(heading + rot);
        particles(p).X(3) = particles(p).X(3) + (T*(velocity + trans)*sin(heading + rot));
        particles(p).X(4) = (velocity + trans)*sin(heading + rot);
    ...
    for m = 1:size(Z.id,2)
        if global_point_landmark_map.isKey(string(Z.id(m))) 
            for p = 1:size(particles,2)
                ...
                R = blkdiag(1,0.001); %EKF scene0655 :0.6,0.01 or 0.5,0.01 ; EKF scene0916: 0.8,0.07 or 0.8,0.05  multi_0916: 0.1, 0.005; one_0655:5,0.001 ; multi_0655: 0.1, 0.005
                %1,0.001
