...
% x_min = [x-2, y-2, normalize_angle(theta-deg2rad(5))]; % x, vx, y, vy
% x_max = [x+2, y+2, normalize_angle(theta+deg2rad(5))];
% particles = create_uniform_particles(x_min, x_max, N);

% Particles generated from normal distribution
mean_state = [x, y, theta]; % x, vx, y, vy
std_state = [3, 3, deg2rad(5)]; % position with 20m 3sig error, and velocity with 5m/s 1sig error. %2, 1, 2, 1
particles_pre = create_gaussian_particles(mean_state, std_state, N_particles); % A array of Particles is created

% First Update
if dead_reckoning
    particles_post = particles_pre;
    weight_particles_post = weight_particles;
    weight_particles_pre = weight_particles;
...
%         Z_ = rotate_measurements_to_global_frame(Z_, control_input(2,timestep));
        weight_particles_pre = weight_particles;
        [particles_post, weight_particles_post] = update(particles_pre, weight_particles, Z_);%, control_input(2,timestep));
        if effective_no(weight_particles_post) < N_particles/2
            indexes = systematic_resampling(weight_particles_post);
            [particles, weight_particles] = resample_from_index(particles_post, weight_particles_post, indexes);
        else
            weight_particles = weight_particles_post;
            particles = particles_post;
        end
    end
end

filter_consistency(timestep).particles = particles_post;
filter_consistency(timestep).weights = weight_particles_post;

plot_on_global_map(timestep, particles_pre, particles_post, weight_particles_post, weight_particles_pre, ground_truth_ego_pos, measurements_global, ax);
% pause(1);
...
function plot_on_global_map(timestep, particles_pre, particles, weights_post, weights_pre, ground_truth_ego_pos, measurements_global, ax)
    legend off;
    ...
    [point_mean, point_var] = point_estimate(particles_pre, weights_pre);
    plot(point_mean(1), point_mean(2), "c*", 'Tag', "point_particles_predicted");
    
    [point_mean, point_var] = point_estimate(particles, weights_post);
...
function [particles] = predict(particles, T, velocity, heading)
    std_velocity = 0.1; %KF both scenes - 1; EKF
    std_heading = deg2rad(1); %KF both scenes - 0.08; EKF
	
    for p = 1:size(particles,2) % Use Parallel toolbox
        trans = T*normrnd(velocity, std_velocity);
        rot = normrnd(heading, std_heading);
        particles(p).X(3) = normalize_angle(rot); % This is very important. Just rot or (rot + particles(p).X(3))????
        % Check if the heading needs to be updated by adding the control input to existing value or just replacing it.
		particles(p).X(1) = particles(p).X(1) + (trans*cos(particles(p).X(3)));
        particles(p).X(2) = particles(p).X(2) + (trans*sin(particles(p).X(3)));

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         particles(p).X(1) = particles(p).X(1) + (T*particles(p).X(2));
%         particles(p).X(2) = particles(p).X(2) + (randn(1)*std_velocity);
%         particles(p).X(3) = particles(p).X(3) + (T*particles(p).X(4));
%         particles(p).X(4) = particles(p).X(4) + (randn(1)*std_velocity);
    end
end
...
function [particles, weights] = update(particles, weights, Z) 
    ...
    for m = 1:size(Z.id,2)
        if global_point_landmark_map.isKey(string(Z.id(m))) 
            for p = 1:size(particles,2)
                ...
                y_prime = land.X(2,1) - particles(p).X(2,1);
                
                h = [sqrt(x_prime^2 + y_prime^2); normalize_angle(atan2(y_prime, x_prime) - particles(p).X(3,1))];
                R = blkdiag(1,0.01); %EKF scene0655 :0.6,0.01 or 0.5,0.01 ; EKF scene0916: 0.8,0.07 or 0.8,0.05  multi_0916: 0.1, 0.005; one_0655:5,0.001 ; multi_0655: 0.1, 0.005
    ...
    for m = 1:size(Z.id,2)
        if ~global_point_landmark_map.isKey(string(Z.id(m))) 
            for p = 1:size(particles,2)
                landmark = struct;
                robot = particles(p).X(:,1);                
                range = sqrt(Z.zpos(1,m)^2 + Z.zpos(2,m)^2);
                bearing = atan2(Z.zpos(2,m), Z.zpos(1,m));
                landmark.X(1,1) = robot(1,1) + (range * cos(robot(3,1) + bearing));
                landmark.X(2,1) = robot(2,1) + (range * sin(robot(3,1) + bearing));
...
function [particles] = create_uniform_particles(x_min, x_max, N)
    ...
    for i = 1:N
        x =[(x_max(1)-x_min(1)).*rand(1)+x_min(1); 
            ...
            normalize_angle((x_max(3)-x_min(3)).*rand(1)+x_min(3))];
...
function [particles] = create_gaussian_particles(mean, std, N)
    ...
    for i = 1:N
        x =[mean(1)+(randn(1)*std(1)); 
            ...
            normalize_angle(mean(3)+(randn(1)*std(3)))];
        particles = [particles, Particle(x)];
    end
end
