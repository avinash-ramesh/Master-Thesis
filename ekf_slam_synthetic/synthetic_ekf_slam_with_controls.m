...
v = VideoWriter('synthetic_implementation\videos\synthetic_ekf_slam_x_vx_y_vy_with_controls.avi');
v.FrameRate = 30;
open(v);
...
if consistency_checks
    save("synthetic_implementation\workspaces\filter_consistency_synthetic_ekf_x_vx_y_vy_with_controls.mat", "filter_consistency");
end
...
function [X_pre, P_pre] = predict(X, P, T, u)
    ...
    sigma_v = 25;
    sigma_theta = deg2rad(15)^2;
    V = [T*cosd(u(2)), -T*u(1)*sind(u(2));
         cosd(u(2))  , -u(1)*sind(u(2))   ;
         T*cosd(u(2)), T*u(1)*cosd(u(2)) ;
    ...
    X_pre = A * X + B * [u(1)*cosd(u(2)); u(1)*sind(u(2))]; % Predict State
